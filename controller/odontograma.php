...
class Controller_Odontograma {
    ...
    public function verifPlan($tipo) {
        try {
            ...
            return ($a->verifOdontograma($_SESSION['id_tratamiento'], 1) && $a->verifOdontograma($_SESSION['id_tratamiento'], $tipo)) ? true : false;
        } catch (Exception $exc) {
            throw $exc->getMessage();
        }
        return true;
    ...
    public function guardarOdontogramaActual() {
        try {
            if (isset($_POST['piezas'])) {
                $piezas = $_POST['piezas'];
                $p = Model_ServicioOdontograma::getInstance()->actualizarOdontogramaEstados($piezas, $_SESSION['id_odontograma']);
                echo $p;
            }
        } catch (Exception $exc) {
            echo "Error: " . $exc->getMessage();
    ...
    public function actualizarTratamientoCurso() {
        try {
            if (isset($_POST['piezas'])) {
                $piezas = $_POST['piezas'];
                $p = Model_ServicioOdontograma::getInstance()->actualizarOdontogramaPrestaciones($piezas, $_SESSION['id_odontograma']);
                echo $p;
            }
        } catch (Exception $exc) {
            echo "Error: " . $exc->getMessage();
